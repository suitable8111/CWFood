...
-(void)markSelected:(BOOL)blnSelected
{
    ...
    if([self.date isDateToday]){
        ...
        self.dateLabel.textColor = (blnSelected)?[UIColor colorWithHex:0x000000]:[UIColor whiteColor];
    }else{
        self.dateLabelContainer.backgroundColor = (blnSelected)?[UIColor whiteColor]: [UIColor clearColor];
        
        self.dateLabel.textColor = (blnSelected)?[UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:1.0]:[self colorByDate];
    }
...
-(UIColor *)colorByDate
{
    return [self.date isPastDate]?[UIColor colorWithHex:0xffffff]:[UIColor whiteColor];
}
